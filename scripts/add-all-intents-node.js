...
// 현재 작업 디렉토리에서 파일 찾기 (경로 문제 해결)
const intentsFile = path.join(process.cwd(), 'data', 'intents.json');

console.log('Reading intents file from:', intentsFile);

// 파일 읽기
let intentsData;
try {
  const fileContent = fs.readFileSync(intentsFile, 'utf-8');
...
function generateAdditionalSentences(intent) {
  const nameKo = intent.name.ko;
  const descKo = intent.description.ko;
  
  // Intent별 맞춤 문장 생성 (간단한 패턴 사용)
  return {
    ko: [
      `${nameKo}에 대해 더 자세히 알려주세요.`,
      `${descKo}에 대한 정보를 얻고 싶습니다.`,
      `${nameKo}에 관해 도움을 요청합니다.`,
      `${descKo}에 대해 설명해 주실 수 있나요?`,
      `${nameKo}에 대한 추가 정보가 필요합니다.`,
      `${descKo}에 관해 질문이 있습니다.`,
      `${nameKo}에 대해 안내해 주세요.`,
      `${descKo}에 대한 조언을 구합니다.`,
      `${nameKo}에 관해 확인이 필요합니다.`
    ],
    en: [
      `Please tell me more about ${intent.name.en}.`,
      `I'd like information about ${intent.description.en}.`,
      `I'm asking for help regarding ${intent.name.en}.`,
      `Could you explain ${intent.description.en}?`,
      `I need additional information about ${intent.name.en}.`,
      `I have questions about ${intent.description.en}.`,
      `Please guide me about ${intent.name.en}.`,
      `I'm seeking advice about ${intent.description.en}.`,
      `I need to confirm about ${intent.name.en}.`
    ],
    ja: [
      `${intent.name.ja}についてもっと詳しく教えてください。`,
      `${intent.description.ja}について情報を得たいです。`,
      `${intent.name.ja}について助けを求めます。`,
      `${intent.description.ja}について説明していただけますか？`,
      `${intent.name.ja}について追加情報が必要です。`,
      `${intent.description.ja}について質問があります。`,
      `${intent.name.ja}について案内してください。`,
      `${intent.description.ja}についてアドバイスを求めます。`,
      `${intent.name.ja}について確認が必要です。`
    ],
    zh: [
      `请详细告诉我关于${intent.name.zh}的信息。`,
      `我想了解关于${intent.description.zh}的信息。`,
      `我请求关于${intent.name.zh}的帮助。`,
      `能解释一下${intent.description.zh}吗？`,
      `我需要关于${intent.name.zh}的更多信息。`,
      `我有关于${intent.description.zh}的问题。`,
      `请指导我关于${intent.name.zh}的信息。`,
      `我寻求关于${intent.description.zh}的建议。`,
      `我需要确认关于${intent.name.zh}的信息。`
    ]
  };
}

// Intent 013~054 추가 및 문장 확장
const existingIds = new Set(intentsData.map(i => i.id));

for (const intentTemplate of intents_013_054) {
  const intentId = intentTemplate.id;
  
  if (existingIds.has(intentId)) {
    // 기존 Intent에 문장 추가
    const intent = intentsData.find(i => i.id === intentId);
    if (intent.sentences && intent.sentences.ko && intent.sentences.ko.length === 3) {
      const additional = generateAdditionalSentences(intentTemplate);
      ...
      console.log(`✅ Added 9 sentences to ${intentId}`);
    } else {
      console.log(`⏭️  Skipped ${intentId} (already has ${intent.sentences?.ko?.length || 0} sentences)`);
    }
  } else {
    // 새 Intent 추가 (문장 9개 추가)
    const additional = generateAdditionalSentences(intentTemplate);
    intentTemplate.sentences.ko.push(...additional.ko);
    intentTemplate.sentences.en.push(...additional.en);
    intentTemplate.sentences.ja.push(...additional.ja);
    intentTemplate.sentences.zh.push(...additional.zh);
    intentsData.push(intentTemplate);
    console.log(`✅ Added new intent ${intentId} with 12 sentences`);
  }
}

// displayOrder로 정렬
intentsData.sort((a, b) => {
  const situationA = parseInt(a.situationId.replace('situation_', ''));
  const situationB = parseInt(b.situationId.replace('situation_', ''));
...
try {
  ...
  console.log(`\n✅ Successfully updated ${intentsFile}`);
  console.log(`   Total intents: ${intentsData.length}`);
  console.log(`   Each intent has 12 sentences (4 languages)`);
} catch (error) {
