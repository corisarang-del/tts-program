...
### 🔧 모듈 레벨 환경 변수 초기화 문제 해결

**문제**:

- `lib/openai.ts`와 `lib/tts.ts`에서 모듈 로드 시점에 `requireEnv('OPENAI_API_KEY')` 호출
- 환경 변수가 없을 때 앱 시작 시점에 에러 발생
- Next.js API Routes에서 런타임 에러로 처리되지 않음

**해결**:

- Lazy initialization 패턴 적용
- OpenAI 클라이언트를 함수 호출 시점에 생성하도록 변경
- 환경 변수 검증을 실제 사용 시점으로 지연

**수정 파일**:

- `lib/openai.ts`: `getOpenAIClient()` 함수 추가, AppError import 및 에러 변환
- `lib/tts.ts`: `getOpenAIClient()` 함수 추가, AppError import 및 에러 변환
- `lib/error-handler.ts`: 환경 변수 에러 처리 로직 추가

**효과**:

- 환경 변수가 없어도 앱이 시작됨
- 실제 API 호출 시점에 명확한 에러 메시지 제공
- 환경 변수 에러를 AppError로 변환하여 일관된 에러 응답
- Next.js 서버리스 환경에 적합한 구조
- 사용자에게 친화적인 에러 메시지 제공 (기술적 세부사항 숨김)
...
3. ✅ 에러 핸들링 타입 안정성 개선
4. ✅ JSON 파싱 로직 개선
5. ✅ 모듈 레벨 초기화 문제 해결 (Lazy initialization)
...
### 🔧 Jest 테스트 환경 설정 문제 해결 (2025-12-25)

**문제**:

- Jest 테스트 실행 시 `ReferenceError: Request is not defined` 에러 발생
- Next.js API Routes 테스트가 실패
- `testEnvironment: 'jest-environment-jsdom'`은 브라우저 환경을 시뮬레이션하므로 Web API가 없음

**해결**:

- `jest.config.js`에서 `testEnvironment`를 `'node'`로 변경
- `jest.setup.js`에 `undici`를 사용한 Web API 폴리필 추가
- `package.json`에 `undici` 및 `@types/node` 추가

**수정 파일**:

- `jest.config.js`: `testEnvironment`를 `'node'`로 변경
- `jest.setup.js`: Web API 폴리필 추가
- `package.json`: `undici` 및 `@types/node` 추가

**효과**:

- Next.js API Routes 테스트가 정상적으로 실행됨
- Web API (Request, Response, Headers)가 Jest 환경에서 사용 가능
- Node.js 18+ 환경과 호환되는 테스트 환경 구성
